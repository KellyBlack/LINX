function m = LINX(S,fk,fi)
% m = LINX(S) computes the Link Importance iNdeX of each flow of a
% compartmental model with the stoichiometric matrix S. Adjacency matrix
...
% this function. 
%
% m = LINX(S,fk,fi) computes the Linx Importance iNdeX of each flow
% for a compartmental model with the stoichiometric matrix S, where fk
...
format compact
disp('Number  Flow      LINX (-1:known -2:infeasible):');
flow_type = sum(S);
for i=1:k
  if ( flow_type(i) == 0 )
    f_from = find( S(:,i) == -1 );
